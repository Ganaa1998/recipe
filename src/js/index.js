...
window.addEventListener("load", e => {
    //Шинээр like model load хийх үед үүсгэнэ.
    if (!state.likes) state.likes = new Like();

    //Лаяк цэсийг гаргах эсэхийг шийдэх
    likesView.toggleLikeMenu(state.likes.getNumberOfLikes());

    //Like ууд байвал тэдгээрийг цэсэнд нэмж харуулна
...
*/
const controlLike = () => {
    // Дарагдсан жорыг авч моделруу хийх

    // 1. Лайкийн моделыг үүсгэнэ.
    if (!state.likes) state.likes = new Like();
    // 2. Одоо харагдаж байгаа жорын id-ыг олж авах
    const currentRecipeId = state.recipe.id;
    // 3. Энэ жорыг Like хийсэн эсэхийг шалгах
    if (state.likes.isLiked(currentRecipeId)) {
        // 4. Like-лсан бол лайкийг нь болиулна.

        state.likes.deleteLike(currentRecipeId);
        // Харагдаж байгаа лайкын цэснээс устгана
        likesView.deleteLike(currentRecipeId);
        // Лайк товчны лайкласан байдлыг болиулах
        likesView.toggleLikeBtn(false);

    } else {
        // 5. Likeлаагүй бол лайклана.
...
elements.shoppingList.addEventListener('click', e => {
    ...
    //Клик хийхсэн li элемэнтийн data-ыг шүүж авлаа
    const id = e.target.closest('.shopping__item').dataset.itemid;

    // Олдсон id-тай орцыг model-оос устгах
